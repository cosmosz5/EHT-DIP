...
def reconstruct_eht_dip(
    ...
    for it in range(1, cfg.num_iter + 1):
        ...
        if (it % (cfg.out_every if out_every is None else out_every) == 0) or it == 1:
            vals = {
                "amp": d_amp, "cp": d_cp, "tv": d_tv, "tsv": d_tsv, "l1": d_l1,
                "flux": d_flux, "comp": d_compact, "void": d_void, "cent": d_cent
            }
            msg = f"[{it:5d}/{cfg.num_iter}] " + " ".join([f"{k}={v.detach().cpu().item():.4g}" for k, v in vals.items()])
            if cfg.learn_global_scale and log_alpha is not None:
